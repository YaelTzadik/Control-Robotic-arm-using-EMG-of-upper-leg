...
int waitForUnoRead() {
  if (Serial1.available()) {
    String press_length_str = Serial1.readString();   // Read a character from UART1
    int press_length_int = press_length_str.toInt();  // Converts String to an integer
    return press_length_int;
  } else
    return NO_UNO_DATA;
}
...
void loop() {
  ...
  if (connected) {  //TRUE if we are already connected to the server
    ...
    int press_length = waitForUnoRead();  //read the data from the uno.
    if (press_length == NO_UNO_DATA) {
      delay(3);
    } else { // press detected.
      Serial.print("Action Length:");
      Serial.println(press_length); 
      int action_id = mod_manager.processCommand(press_length);
