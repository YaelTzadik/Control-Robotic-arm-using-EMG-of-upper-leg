...
void loop() {
  ...
  if (res_left > 0 && res_right > 0) {  // Ensure both results are valid
    Serial.begin(9600);
    String output = String(res_left) + "," + String(res_right);  // Concatenate the two values
    Serial.println(output);  // Send both left and right leg peak results in the format "left_value,right_value"
    Serial.end();
  }
  delay(20);
#endif
